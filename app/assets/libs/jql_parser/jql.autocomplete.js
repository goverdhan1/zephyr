/**
 * @namespace ZEPHYR.JQLAutoComplete
 */
if (typeof ZEPHYR == 'undefined') { var ZEPHYR = {}; }
if (typeof ZEPHYR.JQLAutoComplete == 'undefined') { ZEPHYR.JQLAutoComplete = {}; }

(function (){
    var jql_order_by = [{value:"ORDER BY", displayName:"ORDER BY", fieldName:"ORDER BY"}];
    var jql_operators = [{value:"=", displayName:"=", fieldName:"="}, {value:"!=", displayName:"!=", fieldName:"!="}, {value:"~", displayName:"~", fieldName:"~"}, {value:"<=", displayName:"&lt;=", fieldName:"<="}, {value:">=", displayName:"&gt;=", fieldName:">="}, {value:">", displayName:"&gt;", fieldName:">"}, {value:"<", displayName:"&lt;", fieldName:"<"}, {value:"!~", displayName:"!~", fieldName:"!~"}, {value:"is not", displayName:"is not", fieldName:"is not"}, {value:"is", displayName:"is", fieldName:"is"}, {value:"not in", displayName:"not in", fieldName:"not in"}, {value:"in", displayName:"in", fieldName:"in"}, {value: "was", displayName: "was", fieldName:"was"}, {value: "was not", displayName: "was not", fieldName:"was not"},  {value: "was in", displayName: "was in", fieldName:"was in"}, {value: "was not in", displayName: "was not in", fieldName:"was not in"}, {value: "changed", displayName: "changed", fieldName:"changed"}];
    var jql_logical_operators = [{value:"AND", displayName:"AND", fieldName:"AND"}, {value:"OR", displayName:"OR", fieldName:"OR"}];
    var jql_logical_operators_and_order_by = jql_logical_operators.concat(jql_order_by);
    var jql_order_by_direction = [{value:"ASC", displayName:"ASC", fieldName:"ASC"}, {value:"DESC", displayName:"DESC", fieldName:"DESC"}];
    var empty_operand = [{value:"EMPTY", displayName:"EMPTY", fieldName:"EMPTY", types:["java.lang.Object"]}];
    ...
    jQuery.fn.selectionRange = function(start, end) {
        if (start == null) {
            var res = {
    ...
    ZEPHYR.JQLAutoComplete.init = function (options) {
        ...
        var PARSE_INDICATOR = jQuery("#" + options.errorContainerID);
        ...
        that.responseContainer = jQuery('.jql-autocomplete').find('.suggestions');

        that.dropdownController = {
            element: that.responseContainer,
            displayed : false,
        ...
        that.pushSuggestionsOnHtmlStack = function(suggestions, suggestionNodes, htmlParts, listItemIdentifier, mayNeedParenthesis)
        {
            ...
            for (var i = 0; i < length; i++)
            {
                ...
                if (suggestions[i].value || suggestions[i].fieldName)
                {
                    var resultSug = suggestions[i],
                        resultSugValue = resultSug.value || resultSug.fieldName;
                    if (result && mayNeedParenthesis)
                    {
                        ...
                        actualValueSug = ((result.getNeedsOpenParen()) ? "(" : "") + resultSugValue;
                    }
                    else
                    {
                        actualValueSug = resultSugValue;
                    }
        ...
        that.renderSuggestions = function ( suggestions, moreSuggestions, operatorSuggestions) {
            ...
            if (suggestions instanceof Array)
            {
                ...
                /*if(that.responseContainer.css('display') === 'block'){
                    return;
                }*/

                that.responseContainer
                    .css('top', jQuery('#' + that.fieldID).css('height') + 27)
                    .html(htmlParts.join(''))
                    .find('li').each(function(i) {
                        suggestionNodes[i] = [jQuery(this), suggestionNodes[i]];
                    });

                that.responseContainer.css('display', 'block');
                that.dropdownController.displayed = true;
        ...
        that._makeRequest = function (options) {
            ...
            return this.pendingRequest = ZEPHYR.SmartAjax.makeRequest(requestParams);
        },

        that.delayStatus = {};

        that.delay = function(callback,l) {
            if (that.delayStatus && that.delayStatus.t) {
                clearTimeout(that.delayStatus.t);
                that.delayStatus.t = undefined;
            }
            that.delayStatus.t = setTimeout(callback, l * 1000);
        };
        ...
        that.prepareOperandSuggestions = function(canAutoComplete, fieldName, functionSuggestions, operatorSuggestions, suggestedValue)
        {
                ...
                if (!that.getSavedResponse(fieldNameValueKey))
                {
                    ...
                    that.delay(function()
                    {
                        that._makeRequest({
                            ...
                            {
                                //response = {"results":[{"value":"1-800-TOLLFREE","displayName":"1-800-TOLLFREE  (TOL)"},{"value":"\"Samsung R1\"","displayName":"Samsung R1  (SR)"},{"value":"\"Sandisk Dongle FrontLine Spike & Surge Protector\"","displayName":"Sandisk Dongle FrontLine Spike &amp; Surge Protector  (SD)"},{"value":"Sony","displayName":"Sony  (SONY)"}]};
                                var results;
            ...
            else
            {
                ...
                if(fieldName === 'issue'){
                    var issuesLastOperand = result.getLastOperand() || null;
                    ZEPHYR.IssuePicker.getIssues(issuesLastOperand, function(issues){
                        var issueList = [];

                        issues = JSON.parse(issues).sections[0].issues;

                        for(var i = 0; i < issues.length; i++){
                            issueList.push({
                                value : issues[i].key,
                                displayName : issues[i].keyHtml
                            });
                        }
                        that.renderSuggestionsForOperands(that.stripEscapeCharacters(result.getLastOperand()), issueList, functionSuggestions, operatorSuggestions, suggestionCount);
                    });
                }
                else{
        ...
        that.dispatcher = function (val) {
            ...
            if (result.getNeedsField())
            {
                ...
                that.renderSuggestionsFromMap(that.stripEscapeCharacters(result.getLastFieldName()), jql_field_names, null, true);
            }
            ...
            else if (result.getNeedsOperand())
            {
                ...
                if (!that.isEmptyOnlyOperator(result.getLastOperator()))
                {
                    ...
                    {
                        if (that.equalsIgnoreCase(result.getUnquotedString(jql_field_names[i].fieldName), fieldName) ||
        ...
        that.getSuggestionsForOperators = function(stringVal, list)
        {
            ...
            for (var i = 0; i < jql_field_names.length; i++)
            {
                if (that.equalsIgnoreCase(result.getUnquotedString(jql_field_names[i].fieldName), fieldName) ||
                    (jql_field_names[i].cfid && that.equalsIgnoreCase(jql_field_names[i].cfid, fieldName)))
                {
                    supportedOperators = jql_field_names[i].supportedOperators;
                    break;
                }
            }
        ...
        that.renderSuggestionsForOperands = function(stringVal, ajaxSuggestions, functions, operatorSuggestions, providedSuggestionCount)
        {
            ...
            if (providedSuggestionCount === suggestionCount)
            {
                ...
                if (ajaxSuggestions && ajaxSuggestions.length === 0 && functionsSuggestions && functionsSuggestions.length === 0 && operatorSuggestions && operatorSuggestions.length == 0)
                {
                    that.dropdownController.hideDropdown();
                }
            }
        ...
        that.boldMatchingString = function(stringVal, list)
        {
            ...
            for(var i = 0; i < list.length; i++)
            {
                if (list[i].displayName)
                {
                    ...
                    list[i] = {value:list[i].fieldName, displayName:"<b>" + origVal.substring(0, boldLength) + "</b>" + origVal.substring(boldLength)};
                }
                else
                {
    ...
    that.replaceValue = function(start, end, newValue)
        ...
        that.slimListForFunctionResults = function (stringVal, list, operator)
        {
            ...
            for (var i = 0; i < jql_field_names.length; i++)
            {
                if (that.equalsIgnoreCase(result.getUnquotedString(jql_field_names[i].fieldName), fieldName) ||
                    (jql_field_names[i].cfid && that.equalsIgnoreCase(jql_field_names[i].cfid, fieldName)))
                {
                    supportedTypes = jql_field_names[i].dataType;
                    break;
                }
            }
        ...
        that.updateParseIndicator = function(token)
        {
            if (token.getParseError())
            {
                PARSE_INDICATOR.attr("title", token.getResult().getParseErrorMsg()).removeClass("fa-check-circle").addClass("fa-times");
            }
            else
            {
                PARSE_INDICATOR.attr("title", "").removeClass("fa-times").addClass("fa-check-circle");
            }
        };
        ...
        that.updateColumnLineCount = function()
        {
            ...
            if(that.field[0] === document.activeElement) {
                ...
                totalCharCountToCursor = that.field && that.field[0] && that.field[0].value.length;
            }
            var rowCount = 1;
            var colCount = 1;
        ...
        that.init = function(){
            var that = this;
            that.set(options);
            that.field = that.field || jQuery("#" + that.fieldID);
            // turn off browser default autocomplete
            that.field.attr("autocomplete","off")
            .keyup(function(e){  
                if (!that.disabled && ( !(e.keyCode == 38 || e.keyCode == 40))) {
                    // Fix for ZFJCLOUD-1141             
                    if(this.scrollHeight > 1) {
                        this.style.height = '31px';
                        this.style.height = (this.scrollHeight + 1)+'px';
                    }
                    if (that.responseContainer.is(":visible") && e.keyCode === 13) {
                        e.preventDefault();
                        if (that.evaluateIfActive() && !that.pendingRequest) {
            ...
            .keydown(function (e) {
                var ESC_KEY = 27;
                var jQueryRef = jQuery('#' + that.fieldID);
                // do not clear field in IE
                if ((e.keyCode === ESC_KEY || e.keyCode === 13) && that.responseContainer && that.responseContainer.is(":visible")) {
                    e.preventDefault();
                }
                if (e.keyCode === 13 && !e.ctrlKey && !e.shiftKey && that.responseContainer && !that.responseContainer.is(":visible")) {
                    //Sanjeev: TODO
                    e.preventDefault();
                    that.dropdownController.hideDropdown();
                    jQuery(document).trigger('searchByJQL', [jQueryRef.val()]);
                    e.stopImmediatePropagation();
                }
        ...
        that.addSuggestionControls = function(suggestionNodes) {
            ...
            * @returns {boolean}
            */
            that.evaluateIfActive = function() {
            ...
            */
            ...
            function complete(event) {
                ...
                var fieldRef = jQuery('#' + that.fieldID)[0];
                if(fieldRef.scrollHeight > 1) {
                    fieldRef.style.height = '31px';
                    fieldRef.style.height = (fieldRef.scrollHeight + 1)+'px';
                }
                that.dropdownController.hideDropdown();
            }
        }
    ...
    ZEPHYR.JQLAutoComplete.MyParser = function(jqlReservedWords) {

        var jql_reserved_words = jqlReservedWords;
        return {
            parse: function(input) {
                var token = ZEPHYR.JQLAutoComplete.Token();
    ...
    ZEPHYR.JQLAutoComplete.ParseResult = function() {
    ...
    ZEPHYR.JQLAutoComplete.Token = function() {
        ...
        return {
            ...
            init: function(tokenString)
            {
                ...
                this.result = ZEPHYR.JQLAutoComplete.ParseResult();
                this.result.init();
                this.inParens = 0;
            },
    ...
    _initJQLAutocomplete = function(options) {
        ...
        var jqlFunctionNames = [],
            ...
            JQLAutoComplete = ZEPHYR.JQLAutoComplete.init({
                ...
                errorContainerID: options.errorContainerID,
                autoCompleteUrl: options.autoCompleteUrl
            });

        JQLAutoComplete.parse(jQuery('#' + options.fieldID).text());
        JQLAutoComplete.updateColumnLineCount();
    }

    jQuery(document).on('initJQLAutocomplete', function(ev, options) {
        _initJQLAutocomplete(options);
    });
